...
The **Web** pane has independent tabs like Terminal. Use **+** to open one, click a tab to select it, and close it with **×**. Each conversation keeps its own tabs (up to 12); navigating or opening a link in a new window does not replace another tab. The browser skill exposes `browser_tabs`, `browser_tab_new`, `browser_tab_select` and `browser_tab_close`. `browse`, `read_page`, DOM interaction and browser captures use the conversation's selected tab. A WebView process failure closes its affected tab, leaving other tabs and tools available. Closing the last tab creates a blank replacement.

...
(async()=>{
  ...
  await evaluate(`settingsTab='general';renderSettings();$('auto-continue').checked=true;$('auto-focus-tool').checked=true;$('show-reasoning').checked=false;document.querySelector('#settings-content button').click()`);
  ...
  assert.equal(await evaluate('featureConfig().AutoFocusTool'),true);
  await evaluate(`$('show-reasoning').checked=true;document.querySelector('#settings-content button').click()`);
  ...
  await evaluate("gitMode.value='unified';gitMode.dispatchEvent(new Event('change'))");
  assert.ok(await evaluate("document.querySelector('#git-diff .git-unified .diff-removed').textContent.includes('before')"));
  assert.ok(await evaluate("document.querySelector('#git-diff .git-unified .diff-added').textContent.includes('after')"));
  await evaluate("gitMode.value='split';gitMode.dispatchEvent(new Event('change'))");
  assert.equal(await evaluate("document.querySelector('.metrics').hidden?$('info-toggle').textContent.startsWith('›'):$('info-toggle').textContent.startsWith('⌄')"),true);
  assert.ok(await evaluate("document.querySelectorAll('.message-actions button').length>=2"),'Fork and resume are attached to completed messages');
  await evaluate(`saveResources([${JSON.stringify(path.join(fixtureSources,'report.html'))}])`);
  assert.equal(await evaluate('conversationResources().length'),1);
  assert.ok(await evaluate("$('assets').textContent.includes('report.html')"));
  await evaluate('saveResources([],true)');assert.equal(await evaluate('conversationResources().length'),2);
  ...
  assert.ok(await evaluate(`document.querySelector('.pinned-checklist summary').textContent.includes('1/2')`));
  await evaluate(`document.querySelector('.pinned-checklist summary').click()`);
  assert.equal(await evaluate(`document.querySelector('.pinned-checklist').open`),false);
  await evaluate(`document.querySelector('.pinned-checklist summary button').click()`);
  await waitFor(()=>evaluate(`document.getElementById('pinned-tasks').hidden`));
  await evaluate(`(async()=>{await call('chat.tasks',{id:chatId,dismissed:false});await refresh();renderPinnedTasks();})()`);
  assert.equal(await evaluate(`document.getElementById('pinned-tasks').hidden`),false);
  await captureSettings('workspace-task-checklist');
  await new Promise(r=>setTimeout(r,250));

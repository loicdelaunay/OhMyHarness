...
`%LOCALAPPDATA%\OhMyHarness\harness.db` contient la configuration, l’état de navigation de l’application, les conversations, les messages d’outils, les images et les autorisations permanentes. **EF Core applique les migrations au démarrage** avec `Database.MigrateAsync()` ; migrations et snapshot sont versionnés.
